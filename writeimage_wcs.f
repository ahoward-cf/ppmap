C *************************************************************************
      subroutine writeimage_wcs(outfile,array,nx,ny,ctype1,ctype2, 
     &  crpix1,crpix2,crval1,crval2,cdelt1,cdelt2,crota2,pixel,wl,
     &  sigback,status)

C  Create a FITS primary array with a header containing WCS information.
...
C  Write other optional keywords to the header
      call ftpkys(unit,'BUNIT','MJy/sr',
     &  'Intensity units',status)
     ...
     &  'Wavelength [microns]',status)
      call ftpkye(unit,'SIGBACK',sigback,6,
     &  'Std dev of sky background',status)
